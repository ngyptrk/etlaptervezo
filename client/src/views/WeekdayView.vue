<template>
  <div>
    <div class="d-flex align-items-center justify-content-between mb-3 gap-3 flex-wrap">
      <div class="d-flex align-items-center"><h1 class="m-0">Hét napjai</h1><span class="ms-2 text-warning">({{ filteredRows.length }})</span></div>
      <div class="search-wrap"><i class="bi bi-search search-icon"></i><input v-model="searchWordInput" type="text" class="form-control search-input" placeholder="Keresés napra..." /></div>
    ...
    <div v-else class="list-wrap table-responsive"><table class="table list-table m-0"><thead><tr><th>ID</th><th>Nap</th><th>Művelet</th></tr></thead><tbody><tr v-for="item in filteredRows" :key="item.id"><td>{{ item.id }}</td><td>{{ item.day }}</td><td><div class="d-flex gap-2"><button class="btn btn-sm btn-outline-info" @click="updateHandler(item)"><i class="bi bi-pencil"></i> Módosítás</button><button class="btn btn-sm btn-outline-danger" @click="deleteHandler(item)"><i class="bi bi-trash"></i> Törlés</button></div></td></tr></tbody></table></div>
    <FormWeekday ref="form" :title="formTitle" :item="currentItem" @yesEventForm="yesEventFormHandler" />
...
export default {
  name: "WeekdayView",
  components: { FormWeekday, ConfirmModal },
  data() { return { loading: false, rows: [], mode: "create", currentItem: { id: 0, day: "" }, formTitle: "Új nap felvitele", isOpenConfirmModal: false, confirmTitle: "", confirmMessage: "", confirmAction: null }; },
  computed: {
    ...
    filteredRows() { if (!this.searchword) return this.rows; return this.rows.filter((item) => [String(item.id), item.day].join(" ").toLowerCase().includes(this.searchword)); },
  },
  methods: {
    ...
    async loadAll() { this.loading = true; try { const response = await weekdayService.getAll(); this.rows = response.data ?? []; } finally { this.loading = false; } },
    updateHandler(item) { this.openConfirmModal({ title: "Módosítás megerősítése", message: `Biztosan módosítani szeretnéd ezt a napot: "${item.day}"?`, onConfirm: () => this.startUpdate(item) }); },
    startUpdate(item) { this.mode = "update"; this.formTitle = "Nap módosítása"; this.currentItem = { ...item }; this.$refs.form.show(); },
    createHandler() { this.mode = "create"; this.formTitle = "Új nap felvitele"; this.currentItem = { id: 0, day: "" }; this.$refs.form.show(); },
    deleteHandler(item) { this.openConfirmModal({ title: "Törlés megerősítése", message: `Biztosan törölni szeretnéd ezt a napot: "${item.day}"?`, onConfirm: async () => { await weekdayService.delete(item.id); await this.loadAll(); } }); },
    async yesEventFormHandler({ item, done }) { try { if (this.mode === "create") await weekdayService.create(item); else await weekdayService.update(item.id, item); await this.loadAll(); done(true); } catch (err) { if (err.response && err.response.status === 422) this.$refs.form.setServerErrors(err.response.data.errors); done(false); } },
  },
