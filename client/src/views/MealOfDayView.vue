    ...
    <div class="d-flex align-items-center justify-content-between mb-3 gap-3 flex-wrap">
      ...
      <div class="search-wrap">
        ...
        <input
          ...
          placeholder="Keresés napi étkezésre..."
        />
      </div>
      ...
      <table class="table list-table m-0">
        ...
        <tbody>
          <tr v-for="item in filteredRows" :key="item.id">
            <td>{{ item.id }}</td>
            <td class="mealofday-cell">
              <span>{{ item.meal_of_day }}</span>
  ...
  computed: {
    ...
    filteredRows() {
      ...
      return this.rows.filter((item) =>
        [String(item.id), item.meal_of_day].join(" ").toLowerCase().includes(this.searchword),
      );
    },
  },
  methods: {
    ...
    deleteHandler(item) {
      ...
      this.openConfirmModal({
        ...
        message: `Biztosan törölni szeretnéd ezt a napi étkezést: "${item.meal_of_day}"?`,
        onConfirm: async () => {
          try {
            const id = this.pendingDeleteId;
            this.resetDeleteState();
            if (!id) return;
            const res = await mealOfDayService.delete(id);
    ...
    async yesEventFormHandler({ item, done }) {
      try {
        if (this.mode === "create") await mealOfDayService.create(item);
        else await mealOfDayService.update(item.id, item);
        await this.loadAll();
        done(true);
      } catch (err) {
