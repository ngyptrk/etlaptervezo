  ...
  <div>
    <Modal ref="modal" :title="title" @yesEvent="yesEventHandler">
      <div class="mb-4 row pt-2">
        <label for="meal_of_day" class="col-form-label col-auto pt-1 pe-0">
          Étkezés:
        </label>
        <div class="col">
          <input
            id="meal_of_day"
            v-model="formItem.meal_of_day"
            type="text"
            class="form-control"
            @input="clearError('meal_of_day')"
            required
          />
          <div
            v-if="!serverErrors.meal_of_day"
...
export default {
  ...
  props: {
    title: { type: String, default: "Új étkezés felvitele" },
